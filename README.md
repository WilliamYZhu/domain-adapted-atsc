...
1. Prepare tweet corpus for finetuning. ```python prepare_tweets.py```
...
## Training Workflow
* prepare_tweets.py scrapes tweets and formats them into a tweet corpus.
* pregenerate_training_data.py generates training data from the tweet corpus.
* finetune_on_pregenerated.py finetunes BERT on pregenerated tweet corpus.
* run_glue.py trains model on semeval task 4 dataset.

## Prediction Workflow
* aggregate_and_transform_tweets.py: entire timestamp directory on FireBase => single xml file
* prepare_semeval_datasets.py: single xml file => transformed single xml file
* predict.py: transformed xml file => returns prediction array "preds"
